...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,
        help="Target rows per chunk (overrides --target-mb)",
    )
    ...
    for s in splits:
        total_rows = sum(
            (sp.row_range.length if sp.row_range else sp.file.record_count or 0) for sp in s.splits
        )
        total_bytes = sum(sp.file.file_size or 0 for sp in s.splits)
        print(
        ...
        for sp in s.splits:
            ...
            if sp.row_range is not None:
                rr = sp.row_range
                print(
                    f"    {fname}  rows [{rr.offset:,} – {rr.offset + rr.length - 1:,}]  "
    ...
    for epoch in range(args.epochs):
        ...
        gaps = (
            sorted(unique_ids) != list(range(min(unique_ids), max(unique_ids) + 1))
            if unique_ids
            else False
        )
    ...
    for s in epoch_stats:
        ...
        status = "✓" if ok else "✗"
        print(
            f"  Epoch {s['epoch'] + 1}: {s['rows']:,} rows  {s['batches']:,} batches  "

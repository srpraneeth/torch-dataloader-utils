...
def test_filter_pushdown():
    ...
    batches = list(read_split(shard, format="parquet", filters=pc.field("feature_b") > 30))
...
def test_filter_pushdown_no_matches():
    shard = _shard_from(str(FIXTURES / "sample.parquet"))
    batches = list(read_split(shard, format="parquet", filters=pc.field("feature_b") > 1000))
    total = sum(b.num_rows for b in batches)
    assert total == 0
...
def _write_multi_rg_parquet(path, num_groups: int = 4, rows_per_group: int = 25):
    ...
    tables = [
        pa.table(
            {
                "a": pa.array(
                    list(range(i * rows_per_group, (i + 1) * rows_per_group)), pa.int32()
                ),
                "b": pa.array(
                    [float(x % 10) for x in range(i * rows_per_group, (i + 1) * rows_per_group)],
                    pa.float32(),
                ),
...
def test_get_arrow_filesystem_remote_returns_py_filesystem():
    ...
    with patch(
        "torch_dataloader_utils.format.reader.fsspec.url_to_fs",
        return_value=(mock_fs, "bucket/key.parquet"),
    ):
        with patch("torch_dataloader_utils.format.reader.pafs.FSSpecHandler"):
            with patch(
                "torch_dataloader_utils.format.reader.pafs.PyFileSystem", return_value=mock_py_fs
            ):
